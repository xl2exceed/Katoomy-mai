...
export async function POST(req: NextRequest) {
  try {
    ...
    const hasTransactionalConsent = smsTransactionalConsentRaw !== undefined
      ? smsTransactionalConsentRaw === "true"
      : legacyConsent;
    const hasMarketingConsent = smsMarketingConsentRaw !== undefined
      ? smsMarketingConsentRaw === "true"
      : legacyConsent;
    ...
    if (existingBookingId) {
      ...
      if (netRefOfferId) {
        try {
          ...
          if (offer) {
            const finalPriceCents = Number(priceCents);
            const discountApplied = offer.offer_type === "dollar_off"
              ? offer.amount
              : Math.round(finalPriceCents * offer.amount / (100 - offer.amount));
            ...
            await supabaseAdmin.from("network_offers")
              .update({
            ...
            if (rewardCents > 0 && referral?.id) {
              await supabaseAdmin.from("network_credits").insert({
                business_id: referringBusinessId,
                amount_cents: rewardCents,
                reason: `Referral reward for booking ${bookingId}`,
