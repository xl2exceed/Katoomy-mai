...
export default function UpgradePage() {
  ...
  const isPaidPlan = currentPlan === "premium" || currentPlan === "pro";
  ...
  // Paid users → Stripe Customer Portal (handles upgrades, downgrades, cancels)
  ...
  // Free users → Stripe Checkout (creates new subscription)
  async function handleCheckout() {
    ...
    try {
      const res = await fetch("/api/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ priceId, businessId, email }),
      });
  ...
  const canSelectPlan = (plan: string) => {
    if (isPaidPlan) return false; // paid users use the portal instead
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Paid user banner — sends them to Stripe Portal */}
        {isPaidPlan && (
          <div className="mb-10 max-w-2xl mx-auto bg-purple-50 border border-purple-200 rounded-2xl p-6 text-center">
            <p className="text-purple-800 font-semibold text-lg mb-1">
              You&apos;re on the{" "}
              {currentPlan.charAt(0).toUpperCase() + currentPlan.slice(1)} plan
            </p>
            <p className="text-purple-600 text-sm mb-4">
              To upgrade, downgrade, or cancel — manage your subscription
              directly in the billing portal.
            </p>
            <button
              onClick={handleOpenPortal}
              disabled={loading}
              className="px-8 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-bold shadow transition disabled:opacity-60 disabled:cursor-not-allowed"
            >
              ...
              isPlanCurrent("premium")
                ...
                : selectedPlan === "premium" && canSelectPlan("premium")
                  ...
                  : canSelectPlan("premium")
                    ...
                    : "bg-white border-2 border-gray-200 shadow-lg opacity-75 cursor-default"
            }`}
          >
            {isPlanCurrent("premium") && (
              <div className="absolute -top-5 left-1/2 -translate-x-1/2 px-4 py-1 bg-purple-600 text-white text-sm font-bold rounded-full shadow-lg">
            ...
            <ul className="space-y-3">
              {plans.premium.features.map((feature, i) => (
                <li key={i} className="flex items-start">
                  <svg
                    className={`w-5 h-5 mr-2 flex-shrink-0 ${selectedPlan === "premium" && canSelectPlan("premium") ? "text-orange-400" : "text-purple-600"}`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
          ...
          >
            ...
            <ul className="space-y-3">
              {plans.pro.features.map((feature, i) => (
                <li key={i} className="flex items-start">
                  <svg
                    className={`w-5 h-5 mr-2 flex-shrink-0 ${selectedPlan === "pro" && canSelectPlan("pro") ? "text-orange-400" : "text-purple-600"}`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
        ...
        {/* Checkout Button — free users only */}
        {!isPaidPlan && (
          ...
          <div className="space-y-6 bg-white rounded-xl p-8 shadow-sm">
            <div>
              ...
              <p className="text-gray-600">
                Yes! You can cancel from the billing portal at any time.
            ...
            <div>
              ...
              <p className="text-gray-600">
                Yes! Switch billing cycles anytime from the billing portal.
