...
export default function StaffPwaInstallPrompt() {
  ...
  const [showBanner, setShowBanner] = useState(() => {
    ...
    if (sessionStorage.getItem("staffInstallDismissed")) return false;
    if (
      typeof navigator !== "undefined" &&
      /iphone|ipad|ipod/i.test(navigator.userAgent) &&
      !navigator.userAgent.includes("Chrome")
  ...
  const handleDismiss = () => {
    sessionStorage.setItem("staffInstallDismissed", "1");
    setIsDismissed(true);
    setShowBanner(false);
  };
        ...
        <div className="flex items-center gap-3 flex-1">
          ...
          <div>
            <p className="text-white font-semibold text-sm">
              Install Katoomy Staff
            </p>
            <p className="text-gray-400 text-xs mt-0.5">
              Access your dashboard from your home screen
            </p>
          </div>
        </div>
